...
class HybridWeaponTracker:
    ...
    def _load_model(self):
        """Load YOLO weapon detection model from HuggingFace"""
        try:
            print(f"[HybridTracker] Loading model: {MODEL_NAME}")
            model_path = hf_hub_download(
                repo_id=MODEL_NAME,
                filename="weights/best.pt"
            )
            model = YOLO(model_path)
            print(f"[HybridTracker] Model loaded successfully")
            return model
    ...
    def process_frame(self, frame: np.ndarray) -> Tuple[np.ndarray, Optional[Dict]]:
        ...
        if self.frame_idx % self.yolo_refresh_every == 0:
            self._run_yolo_detection(frame)
        ...
        if self.tracking and self.tracker is not None:
            ...
            if ok:
                ...
                # Choose color based on time since YOLO
                color = (0, 0, 255) if time_since_detection > 0.5 else (0, 255, 0)  # Red if no recent YOLO
                
                # Draw tracker bbox
                cv2.rectangle(annotated_frame, (x, y), (x + w, y + h), color, 2)
                cv2.circle(annotated_frame, (cx, cy), 4, color, -1)
                
                # Label with time remaining if in grace period
        ...
        # Draw YOLO detection if recent
        if self.last_yolo_box is not None:
            x1, y1, x2, y2 = self.last_yolo_box
            cv2.rectangle(annotated_frame, (x1, y1), (x2, y2), (255, 0, 0), 1)  # Blue thin box
        
        return annotated_frame, tracking_info
    
    def _run_yolo_detection(self, frame: np.ndarray):
        ...
        if best_box is not None:
            ...
            # Reinit tracker only if very confident or not tracking
            if best_confidence >= self.conf_threshold_initial or not self.tracking:
                w = x2 - x1
                h = y2 - y1
                bbox = (x1, y1, w, h)
                ...
                print(f"[HybridTracker] 🎯 Detected: {best_class_name} ({best_confidence:.2f})")
            else:
                print(f"[HybridTracker] ✓ Redetected: {best_class_name} ({best_confidence:.2f}), keeping tracker")
        else:
